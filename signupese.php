...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($creation_date > $current_date) {
    $errors[] = "La date de création de l'entreprise ne peut pas être postérieure à la date actuelle.";
  }
  ...
  if (!empty($_FILES["image"]["name"])) {
    $targetDir = "imglogo/entreprise/"; // Specify the directory where you want to store the uploaded files
  ...
  if (empty($errors)) {
    ...
    if ($result->num_rows > 0) {
      ...
      $sql_user = "INSERT INTO utilisateur (email, pass_word, utilisateur_type,date_inscription) VALUES ('$email', '$password', 'entreprise','$currentDate')";
      if ($conn->query($sql_user) === FALSE) {
        ...
        $sql_ecole = "INSERT INTO entreprise (ID_Entreprise,Nom_Entreprise,Logo	,
      Secteur_Activite,	Description,Date_Creation,Localisation,Contact,website	) VALUES 
      ( '$user_id','$school_name', '$targetFile','$secteur', '$description',
      '$date_creation','$location', '$Contact', '$website')";
        if (!move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
          $errors[] = "Désolé, une erreur s'est produite lors du téléchargement de votre fichier.";
        }
        if ($conn->query($sql_ecole) === FALSE) {
          $errors[] = "Erreur lors de l'insertion des données de l'entreprise: " . $conn->error;
        } else {
...
<head>
  ...
  <style>
    * {
    ...
    .btn-warning:hover {
      background-color: rgb(18, 94, 208);
    }
...
<body>
  ...
  <div class="container">
    <header>Inscription pour entreprises</header>
    <img src="icon.jpg">
    ...
    <form action="#" method="post" enctype="multipart/form-data">
      <div class="form first">
        ...
        <div class="details ID">
          <span class="title">Les informations d'entreprise </span>

          <div class="fields">
            <div class="input-field">
              <label>Nom d'entreprise</label>
              <input type="text" name="school_name" placeholder="Entrer nom d'entreprise" required>
            </div>

            <div class="input-field">
              <label>Adresse </label>
              <input type="text" name="location" placeholder="Enter la localisation " required>
            </div>

            <div class="input-field">
              <label>Secteur d'activité</label>
              <select name="secteur" name="secteur" required>
                <option disabled selected>choisir le Secteur d'activité </option>
                <option>coomercial</option>
                <option>industriel</option>
                <option >Technologie de l'information et des communications </option>
                <option >Finance et services bancaires</option>
                <option >Santé et pharmaceutique</option>
                <option >Énergie et environnement</option>
                <option >Industrie manufacturière</option>
                <option >Commerce de détail et commerce électronique</option>
                <option >Éducation et formation</option>
                <option >Tourisme et hôtellerie</option>
                <option >Transport et logistique</option>
                <option >Services professionnels :conseil- juridique- comptabilité- etc.</option>
              </select>
            </div>
            <div class="input-field">
            ...
            <div class="input-field">
              ...
              <input type="text" name="website" placeholder="Entrer votre Site Web" required>
            </div>
