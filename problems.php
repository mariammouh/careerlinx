        ...
        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <li class="nav-item">
                ...
                <div id="collapseUtilities" class="collapse show" aria-labelledby="headingUtilities"
                    ...
                    <div class="bg-white py-2 collapse-inner rounded">
                    <h6 class="collapse-header">Gestion de  platforme :</h6>
                    <a class="collapse-item active" href="problems.php"><i class="fas fa-fw fa-exclamation-triangle"></i> Gestion de problèmes</a>                        <!-- <a class="collapse-item" href="entretiens.php"><i class="fa fa-handshake"></i> PFQ Entretiens </a>
...
<a class="dropdown-item text-center small text-gray-500" href="<?php readALL("notification"); ?>">Fermer</a>
    ...
    <table>
        <tr>
            <th>Titre</th>
            <th>Date premiere apparition</th>
            <th>Description</th>
            <th>Statut</th>
        ...
        // Récupérer les problèmes depuis la base de données
        ...
        if (isset($_GET['message']) && $_GET['message'] == 'success') {
            echo "<p style='color: green; text-align: center;'>Problème ajouté avec succès.</p>";
        }

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>  
                <form action='repondAdmin.php' method='get'>
                  <td><input type='text' id='titreInput' name='titre' value='" . $row["titre"] . "'></td>
                  <td><input type='date' id='dateInput' name='date_premiere_apparition' value='" . $row["date_premiere_apparition"] . "'></td>
                  <td><input type='text' id='descriptionInput' name='description' value='" . $row["description"]. "'></td>
                  <td><input type='text' id='statutInput' name='statut' value='" . urlencode($row["statut"]) . "'></td>
                  <td><input type='date' id='resolutionInput' name='date_resolution' value='" . urlencode($row["date_resolution"]) . "'></td>
                  <td>
                    <button class='button bg' type='submit'>Enregistrer</button>
                  </td>   
                </form>
                <script>
                  document.getElementById('sendButton').addEventListener('click', function() {
                    var titre = document.getElementById('titreInput').value;
                    var datePremiereApparition = document.getElementById('dateInput').value;
                    var description = document.getElementById('descriptionInput').value;
                    var statut = document.getElementById('statutInput').value;
                    var dateResolution = document.getElementById('resolutionInput').value;
              
                    var url = 'repondAdmin.php?id_probleme=" . urlencode($row["id_probleme"]) . "&action=probleme&stajtut=' + encodeURIComponent(statut) + 
                    '&titre=' + encodeURIComponent(titre) + 
                    '&date_premiere_apparition=' + encodeURIComponent(datePremiereApparition) + 
                    '&description=' + encodeURIComponent(description) + 
                    '&date_resolution=' + encodeURIComponent(dateResolution);
            ...
            echo "<tr><td colspan='5'>Aucun problème trouvé.</td></tr>";
        }
        $conn->close();
        ?>
